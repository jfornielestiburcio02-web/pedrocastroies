...
export function UserManagementListView() {
  ...
  if (isLoading) {
    return <div className="flex justify-center p-20"><Loader2 className="h-8 w-8 animate-spin text-[#9c4d96]" /></div>;
  }
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50 border-b sticky top-0 z-10">
                <tr>
                  ...
                  <th className="p-4 text-[10px] font-bold text-gray-400 uppercase">Roles y Subperfiles</th>
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap gap-1">
                        {user.rolesUsuario?.map((r: string) => (
                          <Badge key={r} variant="outline" className="text-[8px] font-bold uppercase border-purple-100 bg-purple-50/50 text-purple-700">
                            {r.replace('Es', '')}
                          </Badge>
                        ))}
                        {user.perfilesAdicionales?.map((p: string) => (
                          <Badge key={p} variant="outline" className="text-[8px] font-bold uppercase border-blue-100 bg-blue-50 text-blue-700">
                            {p}
                          </Badge>
                        ))}
                      </div>
      ...
      <Dialog open={!!designatingTutorId} onOpenChange={() => setDesignatingTutorId(null)}>
        <DialogContent className="max-w-md font-verdana p-0 border-none overflow-hidden">
          <DialogHeader className="bg-[#9c4d96] p-6 text-white shrink-0">
             <DialogTitle className="text-sm font-bold uppercase tracking-widest">Designar Tutoría de Grupo</DialogTitle>
          </DialogHeader>
          <div className="p-8 bg-white space-y-6">
             <Select onValueChange={(val) => handleAssignTutor(designatingTutorId!, val)}>
               <SelectTrigger className="h-12 border-gray-300 font-bold text-sm">
                 <SelectValue placeholder="Elija un grupo..." />
               </SelectTrigger>
               <SelectContent>
                 <SelectItem value="NONE" className="text-red-600">-- Eliminar Tutoría --</SelectItem>
                 {existingCourses.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
               </SelectContent>
             </Select>
          </div>
        </DialogContent>
...
/**
 * Vista de Gestión de Subperfiles / Coordinaciones (Dirección -> Perfil).
 */
export function UserProfilesManagementView() {
  const db = useFirestore();
  const { toast } = useToast();
  const [searchTerm, setSearchTerm] = useState("");
  ...
  const handleToggleSubProfile = (teacherId: string, profile: string, isAssigned: boolean) => {
    ...
    const nextProfiles = isAssigned 
      ? currentProfiles.filter((p: string) => p !== profile)
      : [...currentProfiles, profile];

    updateDocumentNonBlocking(doc(db, 'usuarios', teacherId), { perfilesAdicionales: nextProfiles });
    toast({ 
      title: isAssigned ? "Perfil retirado" : "Perfil asignado", 
      description: `Se ha actualizado la coordinación de ${teacher.nombrePersona}` 
    });
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6 max-w-6xl mx-auto w-full font-verdana">
      <div className="bg-white border rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[600px]">
        <div className="bg-[#f8f9fa] border-b p-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <UserCircle className="h-5 w-5 text-[#9c4d96]" />
            <span className="text-sm font-bold text-gray-700 uppercase">Gestión de Subperfiles y Coordinaciones</span>
          </div>
          <div className="relative w-full md:w-80">
            <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-gray-400" />
            <Input 
        ...
        <ScrollArea className="flex-1">
          <div className="p-4 space-y-4">
            {filteredTeachers.map((teacher) => (
              <div key={teacher.id} className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm flex flex-col md:flex-row gap-6 hover:border-purple-200 transition-all">
                <div className="flex items-center gap-4 min-w-[250px]">
                  <Avatar className="h-12 w-12 border-2 border-white shadow-md">
                    <AvatarImage src={teacher.imagenPerfil} />
                    <AvatarFallback>{teacher.usuario?.substring(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-gray-700 uppercase">{teacher.nombrePersona || teacher.usuario}</span>
                    <span className="text-[10px] text-gray-400 font-bold uppercase">Profesorado del Centro</span>
                  </div>
                </div>

                <div className="flex-1">
                  <Label className="text-[10px] font-bold text-gray-400 uppercase mb-3 block">Perfiles asignados</Label>
                  <div className="flex flex-wrap gap-2">
                    {subProfiles.map(profile => {
                      const isAssigned = teacher.perfilesAdicionales?.includes(profile);
                      return (
                        <button
                          key={profile}
                          onClick={() => handleToggleSubProfile(teacher.id, profile, !!isAssigned)}
                          className={cn(
